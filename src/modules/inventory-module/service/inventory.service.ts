...
export class InventoryService {
   ...
   async findAllInventories(): Promise<Inventory[]> {
    this.logger.log('Finding all inventories');
       try {
           ...
           throw new HttpException('Error getting inventories', HttpStatus.INTERNAL_SERVER_ERROR);
       }
   }
   ...
   async findInventoryById(id: number): Promise<Inventory> {
       ...
       try {
           ...
           this.logger.error(error);
           throw new HttpException('Error getting inventory', HttpStatus.INTERNAL_SERVER_ERROR);
       }
   }
   ...
   async updateInventory(id: number, data: UpdateInventoryData): Promise<void> {
    ...
    async updateStock(id: number, quantity: number): Promise<void> {
       ...
       try {
           ...
           this.logger.log(`Updating stock for inventory with id: ${id} to ${quantity}`);
           await this.inventoryRepository.updateStock(id, quantity);
       } catch (error) {
           this.logger.error(error);
           throw new HttpException('Error updating stock', HttpStatus.INTERNAL_SERVER_ERROR);
       }
   }
   ...
   async searchInventory(filters: {
       ...
       try {
           ...
           this.logger.error(error);
           throw new HttpException('Error searching inventory', HttpStatus.INTERNAL_SERVER_ERROR);
       }
    }
}